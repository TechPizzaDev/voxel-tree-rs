...
pub fn adapter_info_summary(info: &wgpu::AdapterInfo) -> String {
    ...
    let mut summary = format!(
        "backend: {backend:?}, device_type: {device_type:?}, device_pci_bus_id: {device_pci_bus_id}, subgroup_size: {subgroup_min_size}..{subgroup_max_size}, transient_saves_memory: {transient_saves_memory}"
    );
