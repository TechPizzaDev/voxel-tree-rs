...
#let add(short, ..entry) = context {
  ...
  if entry == none {
    ...
    let item = (
      l: long,
      pl: plural,
      lbl: label(short + sym.hyph.point + long.text),
    )
    abbr.update(dct => {
...
#let read-csv(..filename, delimiter) = {
  ...
  read(..filename)
    .split("\n")
    .filter(notempty)
    .map(entry => csv(bytes(entry), delimiter: delimiter).flatten().map(str.trim).filter(notempty))
}
...
#let a(short) = context {
  let (key, dct) = get(short)
  if dct == none { return warn(short) }
  if key in abbr-first.get() { l(key) } else { s(key) }
}
...
#let pla(short) = context {
  ...
  // repr(abbr-first.get())
  if dct == none { return warn(short) }
  if key in abbr-first.get() { pll(key) } else { pls(key) }
}
