...
Anything beyond blocks is practically just technical details for a player, 
but to us it can explain some technical intricacies.   

=== Concurrent Generation
The world is generated chunk-wise and over multiple consecutive steps, 
all on demand as chunks appear in the player's view distance @mcwiki.
Structure origins and biomes are calculated before blocks can be placed. 
Terrain shaping follows, and is a computationally expensive phase 
thanks to various density functions.
The convenient property of these formulas is their continuity, 
given that most of them are multiple octaves of gradient noise 
with some linear arithmetic sprinkled in between.
Density calculations are deterministic and embarrassingly parallel, 
in other words.

Caves and cliffs are fun and all, 
but there is a concurrency problem soon after; 
the decoration post-processing steps may place 
structures, vegetation, and ore -- to name a few. 
The point of congestion is the possibility of decorations 
intersecting multiple chunks, 
which is solved by tracking such dependencies and
finalizing later in a pre-defined order.

=== Rendering
Chunks render only when fully generated. 
This is normal for chunking since it is easier cache and invalidate,
and avoids the substantial overhead of maintaning 
a rapidly shifting preview for incomplete data.